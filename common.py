...
from utils.os_utils_tf import get_arg_parser, str2bool
...
def get_args():
    ...
    parser.add_argument('--env', help='gym env id', type=str, default='Ant-v3')
    parser.add_argument('--num_envs', help='num envs', type=np.int32, default=5)
    ...
    parser.add_argument('--code', help='code using', type=str, default='tf')
    ...
    def atari_args():
        parser.set_defaults(epochs=10, cycles=20, iterations=10, test_rollouts=5, test_timesteps=27000, batch_size=32, warmup=3000)
        parser.add_argument('--atari_wrapper', help='Using wrapper', type=str2bool, default=True)
        parser.add_argument('--clip_rewards', help='Using clip_rewards', type=str2bool, default=True)
    ...
    if args.code=='pytorch':
        from utils.os_utils_pytorch import get_logger
    if args.code=='tf':
        from utils.os_utils_tf import get_logger
    ...
    def q_learning_args():
        ...
        parser.add_argument('--eps_l', help='the beginning percentage of epsilon greedy explorarion', type=np.float32, default=0.05)
        parser.add_argument('--eps_r', help='the final percentage of epsilon greedy explorarion', type=np.float32, default=0.05)
        ...
        parser.add_argument('--clip_score', help='clip_score', type=np.float32, default=5)

        parser.add_argument('--optimizer', help='the optimizer to use', type=str, default='adam', choices=['adam', 'rmsprop'])
        args, _ = parser.parse_known_args()
        if args.optimizer=='adam':
            parser.add_argument('--q_lr', help='the learning rate of value network', type=np.float32, default=1e-4)
    ...
    def dqn_args():
        q_learning_args()
        parser.add_argument('--alpha', help='the learning rate of temperature value', type=np.float32, default=0.9)
    ...
    algorithm_args_collection = {
        ...
        'ircr': ircr_args,
        'rrd': rrd_args,
        'rrd_atari_pytorch':rrd_args,
        'rrd_mujoco_pytorch':rrd_args,
        'rrd_atari_tf':rrd_args,
        'rrd_ddpg':rrd_args,
        'rrd_mujoco_pytorch_xql':rrd_args,
        'rrd_mujoco_pytorch_q':rrd_args,
    }
