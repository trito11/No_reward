    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            # exit()
                ...
                qf1_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_1)) + 0.5 * torch.square(pre_re_1 - torch.mean(mb_eps_rewards)).mean() + args.var_coefficient * r_var1 + args.q_coefficient * torch.mean(0.5*pre_re_1**2-pre_re_1)
                qf2_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_2)) + 0.5 * torch.square(pre_re_2 - torch.mean(mb_eps_rewards)).mean() + args.var_coefficient * r_var2 + args.q_coefficient * torch.mean(0.5*pre_re_2**2-pre_re_2)
