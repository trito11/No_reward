    ...
    class RandomizedReturnDecomposition():
        ...
        def train(self, batch,train_q=False,train_policy=False):
            ...
            with autocast(enabled=False):
                ...
                self.q_pi_1 = self.target_net3(self.rrd_obs_ph, self.pi_act)
                self.q_pi_2 = self.target_net4( self.rrd_obs_ph,self.pi_act)         
                ...
                target1 = (1.0 - self.done_ph) * self.args.gamma * self.q_t_1
                rrd_rews_pred1 = self.q_1 - target1 
                self.rrd1 = rrd_rews_pred1.to(torch.float32).mean(dim=1)
                self.beta=torch.maximum(torch.square(self.rrd_rews_ph).mean(),torch.tensor(10.0).to(device)).to(device)
                wandb.log({"Beta":self.beta})
                self.regularization1= self.args.chi2_coeff* torch.square(rrd_rews_pred1).mean()/self.beta
                ...
                self.q_2=self.q_net2( self.obs_ph,self.rrd_acts_ph) 
                target2 = (1.0 - self.done_ph) * self.args.gamma * self.q_t_3
            ...
            self.train_info_r = {
                # 'R_loss':self.r_loss.item(),
                'Q_total_loss': torch.minimum(self.q_total_loss1,self.q_total_loss2).item()
            }
            if args.rrd_bias_correction:
