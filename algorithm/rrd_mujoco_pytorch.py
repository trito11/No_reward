...
def RRD_mujoco_pytorch(args):
    ...
    class Reward2(nn.Module):
        ...
        def forward(self, obs_ph, acts, next_obs_ph):
            ...
            # Concatenate observation and action inputs
            x = torch.cat([obs_ph, acts, obs_ph-next_obs_ph], dim=-1)
            x = x.to(device).to(torch.float32)
    ...
    class RandomizedReturnDecomposition():
        def __init__(self, args):
            ...
            self.target_net3=QValueNetwork(self.args).to(device)
            self.target_net3.load_state_dict(self.q_net1.state_dict())
            for param in self.target_net3.parameters():
                param.requires_grad = False
            ...
            self.target_net4=QValueNetwork(self.args).to(device)
            self.target_net4.load_state_dict(self.q_net1.state_dict())
            for param in self.target_net4.parameters():
                param.requires_grad = False
        ...
        def train(self, batch,train_q=False,train_policy=False):
            ...
            self.rrd_raw_obs_ph = torch.tensor(batch['rrd_obs']).to( dtype=torch.float32).to(device)
 
            self.rrd_raw_obs_next_ph = torch.tensor(batch['rrd_obs_next']) .to(dtype=torch.float32).to(device)
            self.rrd_acts_ph = torch.tensor(batch['rrd_acts']).to(dtype=torch.float32).to(device) if self.args.env_category!='atari' else one_hot(torch.tensor(batch['rrd_acts']).to(dtype=torch.int64)).to(device)
            self.rrd_rews_ph = torch.tensor(batch['rrd_rews']).to(dtype=torch.float32).to(device)
 
            self.raw_obs_ph = torch.tensor(batch['obs']).to( dtype=torch.float32).to(device)
            self.raw_obs_next_ph = torch.tensor(batch['obs_next']) .to(dtype=torch.float32).to(device)
            self.acts_ph = torch.tensor(batch['acts']).to(dtype=torch.float32).to(device) if self.args.env_category!='atari' else one_hot(torch.tensor(batch['acts']).to(dtype=torch.int64)).to(device)
            self.rews_ph = torch.tensor(batch['rews']).to(dtype=torch.float32).to(device)
            # self.done_ph = torch.tensor(batch['done']).to(dtype=torch.float32).view(-1,self.args.rrd_sample_size,1).to(device)
            self.done_ph = torch.tensor(batch['done']).to(dtype=torch.float32).to(device)
 
 
            noise_size = self.acts_ph.shape
            ...
            with autocast(enabled=False):
                ...
                # print(self.pi.mean.shape)
                # print(self.pi_noise_ph.shape)
                ...
                self.q_pi_1 = self.target_net3(self.obs_ph, self.pi_act)
                self.q_pi_2 = self.target_net4( self.obs_ph,self.pi_act)         
 
                self.q_pi = torch.minimum(self.q_pi_1, self.q_pi_2) - self.alpha * self.pi_log_p
                ...
                self.q_t_3=self.q_net2(self.obs_next_ph, self.pi_next_act)
                self.q_t_4=self.target_net2(self.obs_next_ph, self.pi_next_act)
 
 
                self.q_t_3=  self.q_t_3 - self.alpha * self.pi_next_log_p
                self.q_t_4= self.q_t_4 - self.alpha * self.pi_next_log_p
 
                self.q_1=self.q_net1( self.obs_ph,self.acts_ph) 
                self.q_t=torch.minimum(self.q_t_4,self.q_t_2)
                target1 = (1.0 - self.done_ph) * self.args.gamma * self.q_t
                rrd_rews_pred1 = self.q_1 - target1 
 
 
 
                self.e_reward=self.reward2(self.rrd_obs_ph,self.rrd_acts_ph,self.rrd_obs_next_ph).to(torch.float32)
                self.e_reward2=self.reward2(self.obs_ph,self.acts_ph,self.obs_next_ph).to(torch.float32)
 
 
                self.reward_loss2=criterion(self.rrd_rews_ph, self.e_reward.mean(dim=1))  
 
 
                # self.rrd1 = rrd_rews_pred1.to(torch.float32).mean(dim=1)
 
                # self.regularization1= self.args.chi2_coeff* torch.square(rrd_rews_pred1).mean()/10
                # self.regularization2= self.args.chi2_coeff1*criterion(self.q_t_2, self.q_t_1)
 
                # self.q_loss1 = criterion(self.rrd_rews_ph, self.rrd1)  
                self.q_loss1 = criterion(self.q_1, target1 +self.e_reward2.detach())  
                # self.q_loss_re1 = self.q_loss1 + self.regularization1 + self.regularization2
                ...
                #     rrd_mean1 = rrd_rews_pred1.mean(dim=1,keepdim=True)
 
                #     r_var_single1 = ((rrd_rews_pred1 - rrd_mean1) ** 2).sum(dim=1) / (n - 1)
                #     r_var1 = (r_var_single1 * self.rrd_var_coef_ph / n).mean()
 
                #     # Calculate the total loss incorporating the variance
                #     self.q_total_loss1 = self.q_loss_re1 - r_var1
                # else:
                #     self.q_total_loss1 = self.q_loss_re1           
                ...
                self.q_2=self.q_net2( self.obs_ph,self.acts_ph) 
                target2 = (1.0 - self.done_ph) * self.args.gamma * self.q_t
                # rrd_rews_pred2 = self.q_2 - target2
 
                # self.rrd2 = rrd_rews_pred2.to(torch.float32).mean(dim=1)
 
 
 
                # self.regularization3= self.args.chi2_coeff* torch.square(rrd_rews_pred2).mean()/self.beta
                # self.regularization4= self.args.chi2_coeff1*criterion(self.q_t_3, self.q_t_4)
 
                # self.q_loss2 = criterion(self.rrd_rews_ph, self.rrd2)  
                self.q_loss2 = criterion(self.q_2, target2+self.e_reward2.detach())  
                # self.q_loss_re2 = self.q_loss2 + self.regularization3 + self.regularization4
 
                # self.r_loss=criterion1(self.e_reward1,self.rrd)
 
 
                # wandb.log({'regularization':torch.minimum(self.regularization1,self.regularization3)})
                # wandb.log({'regularization2':torch.minimum(self.regularization2,self.regularization4)})
                ...
                #     # Compute the variance of the predicted rewards
                #     rrd_mean2 = rrd_rews_pred2.mean(dim=1,keepdim=True)
 
                #     r_var_single2 = ((rrd_rews_pred2 - rrd_mean2) ** 2).sum(dim=1) / (n - 1)
                #     r_var2 = (r_var_single2 * self.rrd_var_coef_ph / n).mean()
 
                #     # Calculate the total loss incorporating the variance
                #     self.q_total_loss2= self.q_loss_re2 - r_var2
                # else:
                #     self.q_total_loss2 = self.q_loss_re2         
                ...
                if train_q:
                    ...
                    # scaler.scale(self.q_total_loss1).backward()
                    scaler.scale(self.q_loss1).backward()
                    ...
                    # scaler.scale(self.q_total_loss2).backward()
                    scaler.scale(self.q_loss2).backward()
 
                    torch.nn.utils.clip_grad_value_(self.q_net2.parameters(), 10)
                    scaler.step(self.q_optimizer2)
                    scaler.update()
            ...
            self.train_info_q = {
            'Q_loss': torch.minimum(self.q_loss1,self.q_loss2).item()
            }
            self.train_info = { **self.train_info_q }
            self.step_info = {
                'Q_average': torch.mean(self.q_pi).item()
            ...
            self.train_info_r = {
                # 'R_loss':self.r_loss.item(),
                'Q_total_loss': torch.minimum(self.q_loss1,self.q_loss2).item()
            }
            # if args.rrd_bias_correction:
            #     self.train_info_r['Q_var'] = torch.minimum(r_var1,r_var2).item()
            self.train_info_q = {**self.train_info_q, **self.train_info_r}
            self.train_info = {**self.train_info, **self.train_info_r}
        ...
        def update_target_net(self):
            ...
            self.target_net3.load_state_dict(policy_net1_state_dict)
 
 
            target_net2_state_dict = self.target_net2.state_dict()
            policy_net2_state_dict = self.q_net2.state_dict()
            for key in policy_net2_state_dict:
                target_net2_state_dict[key] = policy_net2_state_dict[key] * (1 - self.args.polyak) + target_net2_state_dict[key] * self.args.polyak
            self.target_net2.load_state_dict(target_net2_state_dict)
            self.target_net4.load_state_dict(policy_net2_state_dict)
        ...
        def step(self, obs, explore=False, test_info=False):
            ...
            self.q_pi_2=self.q_net2( obs_tensor,self.pi_act)
