...
"""
    ...
    double_q_optimizer = optim.Adam(list(double_q_net_1.parameters())+ list(double_q_net_2.parameters()), lr=args.policy_lr)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            # exit()
                ...
                # reward_2 = r1.view(-1) - (1 - mb_dones.flatten()) * args.gamma * (r2).view(-1)
                ...
                qf1_a_values, qf1_next = double_q_net_1(mb_obs, mb_act, mb_obs_next, next_state_actions).view(-1)
                qf2_a_values, qf2_next = double_q_net_2(mb_obs, mb_act, mb_obs_next, next_state_actions).view(-1)


                next_v_value_1 = (1 - mb_dones.flatten()) * args.gamma * (qf1_next).view(-1)

                next_v_value_2 = (1 - mb_dones.flatten()) * args.gamma * (qf2_next).view(-1)


                pre_re_1 = qf1_a_values - next_v_value_1
                pre_re_2 = qf2_a_values - next_v_value_2
                ...
                qf1_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_1))  + args.var_coefficient * r_var1 +  F.mse_loss(qf1_next, min_qf_next_target )
                qf2_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_2))  + args.var_coefficient * r_var2 +  F.mse_loss(qf2_next, min_qf_next_target )
