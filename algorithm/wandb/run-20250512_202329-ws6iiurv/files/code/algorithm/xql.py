...
class Args:
    ...
    beta: float = 0.1
    """the beta value for the reward network"""
        ...
        if global_step > args.learning_starts:
            ...
            # exit()
                ...
                with torch.no_grad():
                    ...
                    v_next = v_net(mb_obs_next)
                    next_v_value = mb_rewards.flatten() + (1 - mb_dones.flatten()) * args.gamma * (v_next).view(-1)
                    

                qf1_a_values = qf1(mb_obs, mb_act).view(-1)
                qf2_a_values = qf2(mb_obs, mb_act).view(-1)

                qf1_loss = torch.nn.functional.mse_loss(qf1_a_values, next_v_value).mean() 
                qf2_loss = torch.nn.functional.mse_loss(qf2_a_values, next_v_value).mean() 
                qf_loss  += qf1_loss + qf2_loss



            
                v_value = v_net(mb_obs).view(-1)
                q_target = qf1_target(mb_obs, mb_act).view(-1)
                q_target = qf2_target(mb_obs, mb_act).view(-1)
                min_qf_target = torch.min(q_target, q_target)

                z = (min_qf_target - v_value)/ args.beta
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("losses/v_loss", v_loss.item()/ args.n_eps, global_step)
                # writer.add_scalar("losses/qf2_loss", qf2_loss.item(), global_step)
