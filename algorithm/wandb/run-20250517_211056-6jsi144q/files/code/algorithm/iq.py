    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            # exit()
                ...
                pre_re_1_t = qf1_a_values.view(-1) - min_qf_next_target
                pre_re_2_t = qf2_a_values.view(-1) - min_qf_next_target
                ...
                qf1_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_1))  +  F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_1_t)) + F.mse_loss(qf1_next, min_qf_next_target - alpha * next_state_log_pi) 
                qf2_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_2))  +  F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_2_t)) + F.mse_loss(qf2_next, min_qf_next_target - alpha * next_state_log_pi ) 
