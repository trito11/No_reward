    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            # exit()
                ...
                qf1_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_1))   + F.mse_loss(qf1_next, min_qf_next_target - alpha * next_state_log_pi) + args.var_coefficient * r_var1
                qf2_loss = F.mse_loss(torch.mean(mb_eps_rewards), torch.mean(pre_re_2))   + F.mse_loss(qf2_next, min_qf_next_target - alpha * next_state_log_pi ) + args.var_coefficient * r_var2
